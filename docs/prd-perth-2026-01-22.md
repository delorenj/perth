...
#### FR-002: Integrated CLI Control (Native `zdrive`)

- **ID:** FR-002
- **Priority:** Must Have
- **Description:** Native CLI commands to create, name, and manage tabs and panes programmatically without external wrappers.
- **Acceptance Criteria:**
    - Commands to create tabs/panes with specific names/layouts.
    - Commands to close/rename specific tabs/panes.
    - API parity with existing `zdrive` functionality but integrated into the `zellij` binary.
...
- How will the database connection be configured securely?
- What is the performance impact of serializing full scrollback history to the DB in real-time?
